...
class Yolov5Detector:
    def __init__(self,
        ...
        input_image_type, input_image_topic, _ = get_topic_type("/camera/color/image_raw", blocking = True)
        self.compressed_input = input_image_type == "sensor_msgs/CompressedImage"
        
        # Initialize subscriber to Image/CompressedImage topic
        input_depth_type, input_depth_topic, _ = get_topic_type("/camera/aligned_depth_to_color/image_raw", blocking = True)
        self.compressed_input = input_image_type == "sensor_msgs/CompressedImage"
        ...
        if self.compressed_input:
            ...
            self.depth_sub = rospy.Subscriber(
                input_depth_topic, CompressedImage, self.depth_callback, queue_size=1
            )
        else:
            self.image_sub = rospy.Subscriber(
