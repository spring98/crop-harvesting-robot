...
class Motor:
    ...
    def degree(self, degree1, degree2, degree3, degree4, degree5, degree6, degree7): # 이 메서드에 들어오는 각도대로 따라가게 하는 메서드
        ...
        current_position_ID7, _, _ = self.packetHandler.read4ByteTxRx(self.portHandler, self.DXL_ID7, self.ADDR_PRESENT_POSITION)
        if current_position_ID7 > 100000000:
            current_position_ID7 = current_position_ID7 - 4294967296
        ...
        current_position_ID1 = current_position_ID1*360/4095
        current_position_ID2 = current_position_ID2*360/4095
        current_position_ID3 = current_position_ID3*360/4095
        current_position_ID4 = current_position_ID4*360/4095
        current_position_ID5 = current_position_ID5*360/4095
        current_position_ID6 = current_position_ID6*360/4095
        current_position_ID7 = current_position_ID7*360/4095
        
        goal_position_ID1 = self.Get_goal_position(degree1, current_position_ID1)
        
        goal_position_ID2 = self.Get_goal_position(degree2, current_position_ID2)
        goal_position_ID3 = self.Get_goal_position(degree3, current_position_ID3)
        print("goal_position3 : %d" % goal_position_ID3)
        goal_position_ID4 = self.Get_goal_position(degree4, current_position_ID4)
        goal_position_ID5 = self.Get_goal_position(degree5, current_position_ID5)
        goal_position_ID6 = self.Get_goal_position(degree6, current_position_ID6)
        goal_position_ID7 = self.Get_goal_position(degree7, current_position_ID7)



        # error calculation
        self.error1 = (goal_position_ID1 - current_position_ID1) # 최대 360 degree
        ...
        self.vk7 = self.vk7 + self.error7*self.dt
        
        self.VELOCITY_ID1 = ( self.kp1 * self.error1 + self.ki1*self.vk1 + self.kd1*(self.error1 - self.error1_p)/self.dt + self.k_anti*(-self.VELOCITY_ID1 + max(min(self.VELOCITY_ID1,self.DESIRED_VELOCITY),-self.DESIRED_VELOCITY)))
        self.VELOCITY_ID2 = ( self.kp2 * self.error2 + self.ki2*self.vk2 + self.kd2*(self.error2 - self.error2_p)/self.dt + self.k_anti*(-self.VELOCITY_ID2 + max(min(self.VELOCITY_ID2,self.DESIRED_VELOCITY),-self.DESIRED_VELOCITY)))
        self.VELOCITY_ID3 = ( self.kp3 * self.error3 + self.ki3*self.vk3 + self.kd3*(self.error3 - self.error3_p)/self.dt + self.k_anti*(-self.VELOCITY_ID3 + max(min(self.VELOCITY_ID3,self.DESIRED_VELOCITY),-self.DESIRED_VELOCITY)))
        self.VELOCITY_ID4 = ( self.kp4 * self.error4 + self.ki4*self.vk4 + self.kd4*(self.error4 - self.error4_p)/self.dt + self.k_anti*(-self.VELOCITY_ID4 + max(min(self.VELOCITY_ID4,self.DESIRED_VELOCITY),-self.DESIRED_VELOCITY)))
        self.VELOCITY_ID5 = ( self.kp5 * self.error5 + self.ki5*self.vk5 + self.kd5*(self.error5 - self.error5_p)/self.dt + self.k_anti*(-self.VELOCITY_ID5 + max(min(self.VELOCITY_ID5,self.DESIRED_VELOCITY),-self.DESIRED_VELOCITY)))
        self.VELOCITY_ID6 = ( self.kp6 * self.error6 + self.ki6*self.vk6 + self.kd6*(self.error6 - self.error6_p)/self.dt + self.k_anti*(-self.VELOCITY_ID6 + max(min(self.VELOCITY_ID6,self.DESIRED_VELOCITY),-self.DESIRED_VELOCITY)))
        self.VELOCITY_ID7 = ( self.kp7 * self.error7 + self.ki7*self.vk7 + self.kd7*(self.error7 - self.error7_p)/self.dt + self.k_anti*(-self.VELOCITY_ID7 + max(min(self.VELOCITY_ID7,self.DESIRED_VELOCITY),-self.DESIRED_VELOCITY)))
        ...
        self.VELOCITY_ID1 = max(min(self.VELOCITY_ID1, self.DESIRED_VELOCITY), -self.DESIRED_VELOCITY)
        self.VELOCITY_ID2 = max(min(self.VELOCITY_ID2, self.DESIRED_VELOCITY), -self.DESIRED_VELOCITY)
        self.VELOCITY_ID3 = max(min(self.VELOCITY_ID3, self.DESIRED_VELOCITY), -self.DESIRED_VELOCITY)
        self.VELOCITY_ID4 = max(min(self.VELOCITY_ID4, self.DESIRED_VELOCITY), -self.DESIRED_VELOCITY)
        self.VELOCITY_ID5 = max(min(self.VELOCITY_ID5, self.DESIRED_VELOCITY), -self.DESIRED_VELOCITY)
        self.VELOCITY_ID6 = max(min(self.VELOCITY_ID6, self.DESIRED_VELOCITY), -self.DESIRED_VELOCITY)
        ...
        self.packetHandler.write4ByteTxRx(self.portHandler, self.DXL_ID2, self.ADDR_GOAL_VELOCITY, int(self.VELOCITY_ID2))
        ...
        self.packetHandler.write4ByteTxRx(self.portHandler, self.DXL_ID7, self.ADDR_GOAL_VELOCITY, int(self.VELOCITY_ID7))
        ...
        print("current_deg1 : %d, goal_deg1 %d,current_deg2 : %d, goal_deg2 %d,current_deg3 : %d, goal_deg3 %d,current_deg4 : %d, goal_deg4, %d,current_deg5 : %d, goal_deg5 %d,current_deg6 : %d, goal_deg6 %d" % (current_position_ID1, goal_position_ID1, current_position_ID2, goal_position_ID2, current_position_ID3, goal_position_ID3, current_position_ID4, goal_position_ID4, current_position_ID5, goal_position_ID5, current_position_ID6, goal_position_ID6 ))
        print( "criteria : %d, error1 : %d, error2 : %d, error3 : %d, error4 : %d, error5 : %d, error6 : %d, error7 : %d" % (CRITERIA, self.error1,self.error2, self.error3, self.error4, self.error5, self.error6, self.error7))
        



    def Is_it_tracking(self):
        if (abs(self.error1) < CRITERIA) and (abs(self.error2) < CRITERIA) and (abs(self.error3) < CRITERIA) and (abs(self.error4) < CRITERIA) and (abs(self.error5) < CRITERIA) and (abs(self.error6) < CRITERIA) and (abs(self.error7) < CRITERIA):
